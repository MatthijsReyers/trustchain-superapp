...
//        TODO: make choice between these
    ...
    private fun setupRecyclerView() {
        pollsAdapter = VotingPollsAdapter { poll ->
            // Navigate to voting fragment for this specific poll (solution or join request)
            val bundle =
                Bundle().apply {
        ...
        lifecycleScope.launch {
            ...
            for (retry in 0..maxRetries) {
                try {
                    ...
                    // Get all solution blocks for this DAO (only for standard features)
                    val allSolutionBlocks = withContext(Dispatchers.IO) {
                        p2playStore.getSolutionBlocksForDaoAndFeature(daoUniqueId)
                    }
                    android.util.Log.d("AllVotingPollsFragment", "loadVotingPolls (Attempt ${retry + 1}): Found ${allSolutionBlocks.size} total standard solution blocks for DAO $daoUniqueId.")
