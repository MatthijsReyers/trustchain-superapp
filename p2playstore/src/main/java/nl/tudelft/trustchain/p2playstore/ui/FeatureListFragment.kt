...
class FeatureListFragment : BaseFragment() {
    ...
    private fun loadFeatureRequests() {
        lifecycleScope.launch {
            try {
                ...
                android.util.Log.d(
                        "FeatureListFragment",
                        "loadFeatureRequests: Found ${featureRequests.size} feature requests for DAO $daoUniqueId"
                )

                // Get all feature solutions for this DAO
                val featureSolutions =
                        withContext(Dispatchers.IO) {
                            p2playStore.getFeatureSolutionsForDao(daoUniqueId)
                        }
                android.util.Log.d(
                        "FeatureListFragment",
                        "loadFeatureRequests: Found ${featureSolutions.size} feature solutions for DAO $daoUniqueId"
                )
                ...
                val featuresWithSolutions =
                        featureRequests.map { featureRequest ->
                            val solutionsForRequest =
                                    solutionsGroupedByFeatureId[featureRequest.FEATURE_REQUEST_ID]
                                            ?: emptyList()
    ...
    private fun navigateToSubmitSolution(featureRequest: FeatureRequestData) {
        ...
        findNavController()
                .navigate(R.id.action_featureListFragment_to_featureSolutionFragment, bundle)
    }
    ...
    private fun navigateToViewSolutions(featureRequest: FeatureRequestData) {
        lifecycleScope.launch {
            if (daoUniqueId.isEmpty()) {
                android.util.Log.e(
                        "FeatureListFragment",
                        "daoUniqueId is empty. Cannot navigate to view solutions."
                )
                Toast.makeText(context, "Error: Missing DAO information.", Toast.LENGTH_SHORT)
                        .show()
                return@launch
            }

            // Get solutions for this specific feature
            val solutionsForFeature =
                    withContext(Dispatchers.IO) {
                        p2playStore.getFeatureSolutionsForDao(daoUniqueId).filter {
                            it.FEATURE_REQUEST_ID == featureRequest.FEATURE_REQUEST_ID
                        }
                    }
            ...
            if (solutionsForFeature.isNotEmpty()) {
                val latestSolution =
                        solutionsForFeature
                                .maxByOrNull {
                                    val solutionBlock =
                                            p2playStore.findProposalBlock(
                                                    daoUniqueId,
                                                    it.SW_UNIQUE_PROPOSAL_ID
                                            )
                                    solutionBlock?.insertTime?.time ?: 0L
                                }
                                ?.let { latestSolutionData ->
                                    val solutionBlock =
                                            p2playStore.findProposalBlock(
                                                    daoUniqueId,
                                                    latestSolutionData.SW_UNIQUE_PROPOSAL_ID
                                            )
                                    if (solutionBlock != null) latestSolutionData
                                    else null // Only return if block found
                                }

                if (latestSolution != null) {
                    android.util.Log.d(
                            "FeatureListFragment",
                            "navigateToViewSolutions: Navigating to vote on latest solution ${latestSolution.SW_UNIQUE_PROPOSAL_ID} in DAO ${daoUniqueId}"
                    )
                    ...
                    findNavController()
                            .navigate(
                                    R.id.action_featureListFragment_to_featureVotingFragment,
                                    bundle
                            )
                } else {
                    android.util.Log.w(
                            "FeatureListFragment",
                            "navigateToViewSolutions called but no *latest* solution block found for feature ${featureRequest.FEATURE_REQUEST_ID} in DAO ${daoUniqueId}. Maybe block not propagated yet?"
                    )
                    Toast.makeText(
                                    context,
                                    "Solution block not yet visible. Please try again in a moment.",
                                    Toast.LENGTH_SHORT
                            )
                            .show()
                }
                ...
                android.util.Log.w(
                        "FeatureListFragment",
                        "navigateToViewSolutions called but no solutions found for feature ${featureRequest.FEATURE_REQUEST_ID} in DAO ${daoUniqueId}. Cannot navigate to voting."
                )
                Toast.makeText(
                                context,
                                "No solutions found for this feature yet.",
                                Toast.LENGTH_SHORT
                        )
                        .show()
            }
